...
-- Description: SQL queries used to analyze sales, profit, inventory,
-- and customer performance.
-- Author: Charles Maina
-- ===============================================================

-- 1️⃣ Top-Performing Products by Sales Volume and Revenue
SELECT 
    p.product_name,
    p.category,
...
-- 2️⃣ Regions Generating the Most Profit
SELECT 
    c.region,
...
-- 3️⃣ Profit Margin per Product and Category
SELECT 
    p.category,
    p.product_name,
    SUM(s.quantity * p.unit_price) AS total_revenue,
...
-- 4️⃣ Inventory Levels vs Sales Performance
SELECT 
    p.product_name,
    i.stock_level,
    SUM(s.quantity) AS total_units_sold,
...
-- 5️⃣ Most Valuable Customers by Total Purchases
SELECT 
    c.customer_name,
    c.region,
